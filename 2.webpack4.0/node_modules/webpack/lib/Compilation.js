...
class Compilation extends Tapable {
	...
	buildModule(module, optional, origin, dependencies, thisCallback) {
		...
		// 调用module.build方法进行编译代码， build其实是利用acorn编译生成抽象语法数(AST)
		// module.build(/* params */)
		module.build(
			this.options,
			this,
	...
	_addModuleChain(context, dependency, onModule, callback) {
		...
		//  this.dependencyFactories中包含有所有的依赖
		const moduleFactory = this.dependencyFactories.get(dependency.constructor);
		// console.log("moduleFactory", moduleFactory)
		// NormalModuleFactory {}   是一个 NormalModuleFactory的实例
		if (!moduleFactory) {
			throw new Error(
		...
		this.semaphore.acquire(() => {
			...
			// 传入执行环境 依赖的模块组成的数组， 触发 beforeResolve钩子
	...
	finish() {
		...
		// console.log("this.hooks.finishModules", this.hooks.finishModules)
	...
	// 生成assets资源并保存到 Compilation.assets中 给webpack写插件的时候会用到
	createModuleAssets() {
